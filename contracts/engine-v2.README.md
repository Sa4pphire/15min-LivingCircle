...
- `originMeters` 为查询点；可选 `originEdgeId` 指定人行道或共享通道。普通人行道最大吸附距离由可选 `maxOriginSnapMeters` 控制，旧输入默认 30 米，真实区域由 Python 固定为 5 米；共享通道仅在估计半宽外 3 米内吸附。普通道路两侧接近且未指定接入边时返回 `AMBIGUOUS_ORIGIN_SIDE`，不能免费换侧。
