...
- 单条过街边等待覆盖值、多入口最短到达、非街边入口接入路径、逐类多源服务范围与边界街段相减；数据不足类别不得输出灰区。
- 真实区域中心点必须位于 `publicWalkableAreasMeters`；封闭地块内部或无法吸附路网时拒绝估算。
...
- 对每类疑似灰区抽查边界街段，并核对入口、合法过街和路网连接；核查设施清单未完成时显示“数据不足”。
